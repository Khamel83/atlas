#!/usr/bin/env python3
"""
Targeted Transcript Processor
Focus on successful podcast types to reach 100 transcripts faster
"""
...
class TargetedTranscriptProcessor:
    def __init__(self):
        self.db_path = "podcast_processing.db"

    def get_pending_episodes_from_successful_podcasts(self, limit: int = 50) -> List[Dict]:
        """Get pending episodes from podcasts that have historically had transcripts"""
        conn = sqlite3.connect(self.db_path)
        conn.row_factory = sqlite3.Row

        # Get podcast IDs that have had transcript success
        successful_podcasts = conn.execute("""
            SELECT DISTINCT p.id, p.name, COUNT(e.id) as transcripts_found
            FROM episodes e
            JOIN podcasts p ON e.podcast_id = p.id
            WHERE e.transcript_found = 1
            GROUP BY p.id, p.name
            HAVING transcripts_found >= 1
            ORDER BY transcripts_found DESC
        """).fetchall()

        if not successful_podcasts:
            print("No previously successful podcasts found")
            return []

        # Get pending episodes from these successful podcasts
        podcast_ids = [str(p['id']) for p in successful_podcasts]
        placeholders = ','.join(podcast_ids)
    ...
    def process_batch(self, limit: int = 50) -> Dict:
        """Process a batch of episodes from successful podcasts"""
        print(f"\n🎯 TARGETED TRANSCRIPT PROCESSOR")
        print("=" * 50)
        print(f"Processing {limit} episodes from successful podcast sources")
        print()

        episodes = self.get_pending_episodes_from_successful_podcasts(limit)
        if not episodes:
            print("✅ No pending episodes from successful podcasts!")
...
if __name__ == "__main__":
    processor = TargetedTranscriptProcessor()
    ...
    # Process targeted batches to reach 100
    target = 100
    while current_count < target:
        batch_size = min(75, target - current_count + 10)  # Get a few extra

        print(f'\n🎯 Processing {batch_size} episodes from successful podcasts...')
        ...
        if result['successful'] == 0:
            print('⚠️ No transcripts found in this batch, trying different approach...')
            break

    print(f'\n🎉 ACHIEVED {current_count} TRANSCRIPTS!')

    if current_count >= 100:
        print('🚀 NOW PUSHING THE REST OF THE EPISODES!')

        # Continue processing all remaining episodes after hitting 100
        print('\n🔄 CONTINUING WITH ALL REMAINING EPISODES...')

        # Switch back to the fast processor for the rest
        from fast_transcript_processor import FastTranscriptProcessor
        fast_processor = FastTranscriptProcessor()

        remaining_episodes = 500  # Process in large chunks
        while True:
            result = fast_processor.process_batch(remaining_episodes)
            if result['successful'] == 0:
                print('✅ Processing complete - no more transcripts found')
                break

            # Check final count
            conn = sqlite3.connect('podcast_processing.db')
            final_count = conn.execute('SELECT COUNT(*) FROM episodes WHERE transcript_found = 1').fetchone()[0]
            conn.close()

            print(f'📊 Total transcripts in database: {final_count}')

            if result['total'] < remaining_episodes:
